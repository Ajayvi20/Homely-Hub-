...
import {propertyActions} from "../../store/Property/property-slice.js";
...
const PropertyList = () => {
  ...
  const lastPage = Math.ceil(totalProperties / 8); // Calculate the last page based on total properties and limit per page

  const propertyListRef = useRef(null);

  useEffect(() => {  //fetch the properties for `currentPage` here and set them above.
    const fetchProperties = async (page) => {
      dispatch(propertyActions.updateSearchParms({
        page: page.page,
        limit: 8
      })
    ); // Set loading state to true
      dispatch(getAllProperties(page.page)) // Fetch properties for the current page
    };
    fetchProperties(currentPage); // Call the function with the current page
    ...
    <>
      ...
      <div className="pagination">
        ...
        <button
          className="next_btn"
          onClick={() => setCurrentPage((prev) => ({ page: prev.page + 1 }))}
          disabled={properties.length < 8 || currentPage.page === lastPage}
        >
